...
import json
import torch
from retrieval_parent import build_child_corpus, ExpansionIndex,HybridSearcher, DenseRetrieval, ParentAwareReranker,pack_results_with_context
...
def run(arg):
    ...
    input_path = arg.rag_data  # child jsonl 또는 glob
    corpus = build_child_corpus(input_path)

    df = pd.read_csv(arg.query_data)
    queries = {row["ID"]: row["Question"] for i, row in df.iterrows()}
    # Parent-child expansion & parent head index
    xp = ExpansionIndex(getattr(arg, "expanded_jsonl", "") or None, getattr(arg, "articles_jsonl", "") or None)
    ...
    # 리랭킹 모델 선언
    reranker = ParentAwareReranker(xp, model_name=arg.reranker, device="cuda")
    ...
    results = searcher.search(
        ...
        alpha=arg.search_alpha,                # 하이브리드 점수 가중치
    ...
    # child+expanded+parent를 한 번에 패킹
    packed_all = pack_results_with_context(
        results, corpus, xp,
        top_n_per_query=arg.final_topk,  # LLM에 줄 문서 수
        preview_chars=900                # 너무 길면 잘라서 토큰 예산 보호
    )
    for qid, cand in results.items():
        hits = packed_all.get(qid, [])
        # 보기 좋게 Head/Child/Expanded를 한 묶음 문자열로 만듦
        doc_texts = [
            f"[{i+1}] Head: {h.get('parent_head','')}\n"
            f"Child: {h.get('child_text','')}\n"
            f"Expanded: {h.get('expanded_text','')}"
    ...
    for q in tqdm(df['Question'], desc="Inference"):
        ...
        t_sel = time.perf_counter() ; print(f"[prof] select: {t_sel - t0:.3f}s", flush=True)
    ...
    # Ollama 응답 받기
        ...
        t_gen = time.perf_counter(); print(f"[prof] generate: {t_gen - t_sel:.3f}s", flush=True)

        # 토큰/초 확인(ollama가 메트릭을 주는 경우)
        ec = resp.get("eval_count"); ed = resp.get("eval_duration")
        if ec and ed:
...
if __name__ == "__main__":
    ...
    # RAG 검색 관련 입력
    parser.add_argument("--rag_data", type=str, required=True, help="jsonl 형식의 RAG 문서 파일 경로")
    parser.add_argument("--query_data", type=str, required=True, help="질문 CSV 파일 경로")
    parser.add_argument("--output", type=str, default="output.csv", help="예측 결과 저장 경로")
    parser.add_argument("--expanded_jsonl", type=str, default="", help="(optional) chunks_child_expanded_*.jsonl 경로 또는 glob")
    parser.add_argument("--articles_jsonl", type=str, default="", help="(optional) articles_*.jsonl 경로 또는 glob")

    # 모델 경로들
    parser.add_argument("--embedding_path", type=str, default="intfloat/multilingual-e5-base", help="SentenceTransformer 임베딩 모델 경로")
    parser.add_argument("--reranker", type=str, default="BAAI/bge-reranker-base", help="CrossEncoder 재랭커 모델 경로")

    # 검색 하이퍼파라미터
    parser.add_argument("--search_alpha", type=float, default=0.7, help="dense 가중치 (alpha)")
    ...
    parser.add_argument("--final_topk", type=int, default=5, help="최종 top-k (재랭커 이후)")

    # Ollama 설정
    parser.add_argument("--model_tag", type=str, required=True, help="Ollama에서 사용할 모델 태그 이름 (예: bllossom-8b)")
